...
class TestSmartMoneySkip(unittest.TestCase):
    ...
    def test_process_market_holders_skips_on_none(self, mock_sleep, mock_db, mock_client_class, mock_goldsky_class):
        # Mock HoldersClient.fetch_holders to return None
        mock_goldsky = mock_goldsky_class.return_value
        mock_goldsky.fetch_holders_subgraph.return_value = None
    ...
    @patch('smart_money_scraper.time.sleep', return_value=None)
    def test_process_market_holders_saves_on_success(self, mock_sleep, mock_save, mock_db, mock_client_class, mock_goldsky_class):
    ...
    @patch('smart_money_scraper.save_holders_batch')
    @patch('smart_money_scraper.time.sleep', return_value=None)
    def test_process_market_holders_saves_on_empty(self, mock_sleep, mock_save, mock_db, mock_client_class, mock_goldsky_class):
        mock_goldsky = mock_goldsky_class.return_value
        mock_goldsky.fetch_holders_subgraph.return_value = []
        mock_client = mock_client_class.return_value
        mock_client.fetch_holders.return_value = []

        result = process_market_holders_worker("market_empty")
